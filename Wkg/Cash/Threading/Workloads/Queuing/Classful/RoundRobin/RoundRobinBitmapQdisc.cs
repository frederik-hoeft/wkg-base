    ...
    where THandle : unmanaged
{
    ...
    protected override bool TryDequeueInternal(int workerId, bool backTrack, [NotNullWhen(true)] out AbstractWorkloadBase? workload)
    {
        ...
        if (IsEmptyInternal)
        {
            ...
            DebugLog.WriteDiagnostic($"{this} qdisc is known to be empty, taking shortcut and returning false.");
            workload = null;
            return false;
        }
        ...
        while (!IsEmptyInternal)
        {
            ...
            do
            {
                if (i != 0)
                {
                    ...
                    DebugLog.WriteDebug($"{this}: emptiness bit map changed while attempting to declare child {qdisc} as empty. Attempts so far {i}. Resampling...");
                ...
                DebugLog.WriteDiagnostic($"{this}: child {qdisc} seems to be empty. Updating emptiness bit map.");
                i++;
            } while (!_dataMap.TryUpdateBitUnsafe(index, token, isSet: false));
            DebugLog.WriteDebug($"{this}: Emptiness state of child qdisc {qdisc} changed to empty.");
        }
    ...
    public override bool TryAddChild(IClassifyingQdisc<THandle> child)
    {
        ...
        DebugLog.WriteDiagnostic($"Trying to add child qdisc {child} to round robin qdisc {this}.");
