        ...
        lock (_syncRoot)
        {
            for (int i = 0; i < _children.Length; i++)
            {
                if (_children[i].Handle.Equals(handle))
                {
                    ...
                    DebugLog.WriteDiagnostic($"Enqueued workload {workload} to child qdisc {_children[i]}.");
                    return true;
                }
                if (_children[i].TryEnqueueByHandle(handle, workload))
                {
                    _dataMap[i] = true;
                    DebugLog.WriteDiagnostic($"{this}: enqueued workload {workload} to child {_children[i]}.");
                    return true;
                }
            }
        }
