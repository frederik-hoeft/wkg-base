    ...
    where TWorkload : AwaitableWorkload, IWorkload
{
    ...
    {
        DebugLog.WriteDiagnostic($"awaiting workload: {_workload}. installing continuation for await");
    ...
    {
        DebugLog.WriteDiagnostic($"GetResult invoked for workload: {_workload}");
...
internal static class WorkloadAwaiterCore
{
    internal static void ValidateEnd(AwaitableWorkload workload)
    {
        ...
        {
            DebugLog.WriteDiagnostic($"workload: {workload} is not completed. blocking until completion");
