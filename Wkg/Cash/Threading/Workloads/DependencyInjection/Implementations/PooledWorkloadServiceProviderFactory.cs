...
namespace Cash.Threading.Workloads.DependencyInjection.Implementations;
...
public class PooledWorkloadServiceProviderFactory : WorkloadServiceProviderFactory, IWorkloadServiceProviderFactory
{
    ...
    {
        DebugLog.WriteDiagnostic("Renting a workload from the PooledWorkloadServiceProvider pool.");
        ...
        try
        {
            ...
            if (provider is null)
            {
                DebugLog.WriteError("PooledWorkloadServiceProvider pool is corrupted! Got a non-negative index, but the provides at that index is null. Please report this bug.");
                return new PooledWorkloadServiceProvider(this, _serviceFactories);
            }
            return provider;
        }
    ...
    {
        DebugLog.WriteDiagnostic("Returning a provider to the PooledWorkloadServiceProvider pool.");
