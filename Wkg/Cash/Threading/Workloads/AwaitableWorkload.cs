...
public abstract class AwaitableWorkload : AbstractWorkloadBase
{
    ...
    {
        DebugLog.WriteDiagnostic($"{this}: cancellation token registration indicates cancellation request.");
        TryCancel();
    }
    ...
    {
        DebugLog.WriteDiagnostic($"{this}: Attempting to cancel workload.");
        // fast and easy path. we can simply check against the base implementation as we don't care about the result
        if (base.IsCompleted)
        {
            // already completed, nothing to do
            DebugLog.WriteDiagnostic($"{this}: Workload is already completed, nothing to do.");
            return false;
        }
        ...
        if (Atomic.TryTestAnyFlagsExchange(ref _status, WorkloadStatus.Canceled, CommonFlags.PreExecution))
        {
            ...
            DebugLog.WriteDiagnostic($"{this}: Successfully canceled workload from pre-execution state.");
            ...
            do
            {
                ...
                if (qdisc is null)
                {
                    DebugLog.WriteDiagnostic($"{this}: Workload is not bound to a qdisc, nothing to do.");
                    // no qdisc, we're done
                    return true;
                }
                // attempt to remove the workload from the qdisc
                if (qdisc.TryRemoveInternal(this))
                {
                    ...
                    UnbindQdiscUnsafe();
                    DebugLog.WriteDiagnostic($"{this}: Successfully removed workload from qdisc.");
                    return true;
                }
            ...
            DebugLog.WriteDiagnostic($"{this}: Failed to remove workload from qdisc. The qdisc does not support removal of workloads.");
            return true;
        }
        ...
        if (Interlocked.CompareExchange(ref _status, WorkloadStatus.CancellationRequested, WorkloadStatus.Running) == WorkloadStatus.Running)
        {
            ...
            if (cts is not null)
            {
                lock (cts)
                {
                    ...
                    {
                        DebugLog.WriteDiagnostic($"{this}: Requesting cancellation of workload cancellation token.");
                        cts.Cancel();
                    }
                }
            }
            // it is the responsibility of the workload to check for cancellation
            DebugLog.WriteDiagnostic($"{this}: Successfully requested cancellation of workload.");
            return true;
        }
        DebugLog.WriteDiagnostic($"{this}: Failed to request cancellation of workload.");
    ...
    {
        DebugLog.WriteDiagnostic($"{this}: Forcing internal cancellation.");
        ...
        {
            DebugLog.WriteDiagnostic($"{this}: Successfully forced internal cancellation.");
        ...
        {
            DebugLog.WriteDiagnostic($"{this}: Failed to force internal cancellation. Status is '{Status}'.");
        }
    ...
    {
        DebugLog.WriteDiagnostic($"{this}: Attempting to bind workload to qdisc.");
        ...
        {
            DebugLog.WriteDiagnostic($"{this}: Failed to bind workload to qdisc. The qdisc is in a completion state and the sentinel is set.");
            return false;
        }
        ...
        if (Atomic.TryTestAnyFlagsExchange(ref _status, WorkloadStatus.Scheduled, WorkloadStatus.Created | WorkloadStatus.Scheduled))
        {
            ...
            if (result)
            {
                DebugLog.WriteDiagnostic($"{this}: Successfully bound workload to qdisc.");
            }
            else
            {
                DebugLog.WriteWarning($"{this}: Failed to bind workload to qdisc. The workload is already running, has been canceled, or another thread was faster.");
            }
            return result;
        }
        ...
        {
            DebugLog.WriteDiagnostic($"{this}: Failed to bind workload to qdisc. The workload was canceled before it could be bound.");
            SetCanceledResultUnsafe();
        }
        else
        {
    ...
    {
        DebugLog.WriteDiagnostic($"{this}: Attempting to execute workload.");
        ...
        if (Interlocked.CompareExchange(ref _status, WorkloadStatus.Running, WorkloadStatus.Scheduled) == WorkloadStatus.Scheduled)
        {
            ...
            DebugLog.WriteDiagnostic($"{this}: Successfully transitioned to running state.");
            UnbindQdiscUnsafe();
            try
            {
                if (!TryExecuteUnsafeCore(out preTerminationStatus))
                {
                    ...
                    DebugLog.WriteWarning($"{this}: Workload is in an invalid state. This is a bug. Status was '{preTerminationStatus}' after execution.");
                    goto FAILURE;
                }
            }
            catch (WorkloadCanceledException)
            {
                ...
                SetCanceledResultUnsafe();
                DebugLog.WriteDiagnostic($"{this}: Execution was canceled with a {nameof(WorkloadCanceledException)}.");
            }
            catch (Exception ex)
            {
                Interlocked.Exchange(ref _status, WorkloadStatus.Faulted);
                SetFaultedResultUnsafe(ex);
                DebugLog.WriteDiagnostic($"{this}: Execution finished faulted with '{ex.GetType().Name}: {ex.Message}'");
            }
            return true;
        }
    FAILURE:
    ...
    internal void ContinueWithCore(IWorkloadContinuation continuation)
    {
        ...
        {
            DebugLog.WriteDiagnostic($"{this}: Capturing synchronization context for continuation.");
        ...
        else if (_continuationOptions.FlowExecutionContext)
        {
            ...
            DebugLog.WriteDiagnostic(_continuationOptions.FlowExecutionContext
                ...
                : $"{this}: Not flowing execution context for continuation.");
    ...
    private bool SpinThenBlockingWait(int millisecondsTimeout, CancellationToken token)
    {
        ...
        for (int i = 0; i < spinCount; i++)
        {
            ...
            if (IsCompleted)
            {
                DebugLog.WriteDiagnostic($"{this}: workload completed during spin wait.");
                return true;
            }
        }
        DebugLog.WriteDiagnostic($"{this}: workload did not complete during spin wait. entering blocking wait.");
