...
public abstract class AbstractWorkloadBase
{
    ...
    internal virtual void AddOrRunInlineContinuationAction(object continuation, bool scheduleBeforeOthers = false)
    {
        ...
        {
            DebugLog.WriteDiagnostic($"{this}: Failed to add continuation. Executing inline.");
    ...
    internal virtual void AddOrRunContinuation(IWorkloadContinuation continuation, bool scheduleBeforeOthers = false, bool runInline = false)
    {
        if (!TryAddContinuation(continuation, scheduleBeforeOthers))
        {
            DebugLog.WriteDiagnostic($"{this}: Failed to add continuation. Invoking on context.");
            if (runInline)
            {
    ...
    {
        DebugLog.WriteDiagnostic($"{this}: Attempting to add continuation.");
        // fast and easy path
        if (IsCompleted)
        {
            DebugLog.WriteDiagnostic($"{this}: Workload is already completed, not scheduling continuation.");
        ...
        {
            DebugLog.WriteDiagnostic($"{this}: Successfully set continuation.");
            // great, we were the first to set the continuation
            return true;
        }
        DebugLog.WriteDiagnostic($"{this}: Continuation already set, attempting to append to list.");
    ...
    private protected virtual bool TryAddContinuationComplex(object continuation, bool scheduleBeforeOthers)
    {
        ...
        {
            DebugLog.WriteDiagnostic($"{this}: Upgrading continuation to list.");
        ...
        if (currentContinuation is List<object?> list)
        {
            lock (list)
            {
                ...
                if (!ReferenceEquals(Volatile.Read(in _continuation), s_workloadCompletionSentinel))
                {
                    ...
                    {
                        DebugLog.WriteDiagnostic($"{this}: List is about to grow, running cleanup.");
                        list.RemoveAll(o => o == null);
                    }

                    DebugLog.WriteDiagnostic($"{this}: Adding continuation to list.");
        ...
        DebugLog.WriteDiagnostic($"{this}: Failed to add continuation (sentinel was set).");
    ...
    internal virtual void InternalRunContinuations(int workerId)
    {
        ...
        {
            case null:
                DebugLog.WriteDiagnostic($"{this}: No continuations to run.");
                return;
            case Action singleContinuation:
                DebugLog.WriteDiagnostic($"{this}: Running inline continuation.");
                singleContinuation.Invoke();
                return;
            case IWorkloadContinuation workloadContinuation:
                DebugLog.WriteDiagnostic($"{this}: Running workload continuation.");
            ...
            case List<object?> list:
            {
                ...
                DebugLog.WriteDiagnostic($"{this}: Running {list.Count(c => c is not null)} continuations.");
                foreach (object? continuation in list)
                {
                    switch (continuation)
                    {
                        ...
                        case null:
                            break;
                        default:
                            DebugLog.WriteError($"{this}: Invalid continuation type '{continuation.GetType().Name}'. This is a bug. Please report this issue.");
                            break;
                    }
                }
                return;
        ...
        DebugLog.WriteError($"{this}: Invalid continuation type '{continuations.GetType().Name}'. This is a bug. Please report this issue.");
    }

    internal virtual void RemoveContinuation(object continuation)
    {
        DebugLog.WriteDiagnostic($"{this}: Attempting to remove continuation.");
        object? currentContinuation = Volatile.Read(in _continuation);
        if (ReferenceEquals(currentContinuation, s_workloadCompletionSentinel))
        {
            DebugLog.WriteDiagnostic($"{this}: Continuation sentinel was set, nothing to do.");
            return;
        }
        ...
        if (list is null)
        {
            if (!ReferenceEquals(Interlocked.CompareExchange(ref _continuation, new List<object>(), continuation), continuation))
            {
                ...
                DebugLog.WriteDiagnostic($"{this}: Lost race to replace continuation with list while removing continuation.");
            }
            else
            {
                DebugLog.WriteDiagnostic($"{this}: Successfully removed single continuation.");
        ...
        if (list is not null)
        {
            lock (list)
            {
                if (!ReferenceEquals(Volatile.Read(in _continuation), s_workloadCompletionSentinel))
                {
                    DebugLog.WriteDiagnostic($"{this}: Removing continuation from list.");
        ...
        DebugLog.WriteDiagnostic($"{this}: Continuation was removed, not found, or sentinel was set.");
    }
}
