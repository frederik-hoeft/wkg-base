...
public class CashVsTpl
{
    ...
    public static ClassfulWorkloadFactory<int> CreateLockingFactory(int concurrency, int depth, int branchingFactor)
    {
        ...
        return WorkloadFactoryBuilder.Create<int>()
            ...
            .UseClassfulRoot<PrioFast<int>>(1, root => ConfigureLockingLevel(root, depth - 1, branchingFactor, handleCounter));
    }

    private static void ConfigureLockingLevel(PrioFast<int> builder, int remainingDepth, int branchingFactor, HandleCounter nextHandle)
    {
        ...
        else
        {
            for (int i = 0; i < branchingFactor; i++)
            {
                ...
                nextHandle.Handle++;
                builder.AddClassfulChild<PrioFast<int>>(handle, i, child => ConfigureLockingLevel(child, remainingDepth - 1, branchingFactor, nextHandle));
            }
        }
    }
    ...
    public static ClassfulWorkloadFactory<int> CreateOptimizedFactory(int concurrency, int depth, int branchingFactor)
    {
        ...
        return WorkloadFactoryBuilder.Create<int>()
            .UseWorkloadDispatcher<BoundedWorkloadDispatcherFactory>(dispatcher => dispatcher
                .UseMaximumConcurrency(concurrency))
            .UseClassfulRoot<PrioFast<int>>(1, root => ConfigureOptimizedLevel(root, depth - 1, branchingFactor, handleCounter));
    }

    private static void ConfigureOptimizedLevel(PrioFast<int> builder, int remainingDepth, int branchingFactor, HandleCounter nextHandle)
    {
        ...
        else
        {
            for (int i = 0; i < branchingFactor; i++)
            {
                int handle = nextHandle.Handle;
                nextHandle.Handle++;
                builder.AddClassfulChild<PrioFast<int>>(handle, i, child => ConfigureOptimizedLevel(child, remainingDepth - 1, branchingFactor, nextHandle));
            }
        }
    }
